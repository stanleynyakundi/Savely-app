...
export async function initiateSTKPush(phoneNumber, amount, accountReference) {
    try {
        ...
        return {
            success: true,
            reference: data.CheckoutRequestID || data.reference || data.transaction_id || data.id,
 ...
 * @returns {Promise<Object>} PayHero response
 */
export async function withdrawToMobile(phoneNumber, amount, networkCode, accountReference) {
    try {
        console.log("Initiating withdrawal:", {
            ...
            networkCode,
            accountReference,
            apiUrl: `${PAYHERO_API_BASE_URL}/withdraw`,
        });

        const response = await fetch(
            `${PAYHERO_API_BASE_URL}/withdraw`,
            {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
        ...
        return {
            success: true,
            reference: data.checkout_request_id || data.merchant_reference || data.reference,
 ...
 * @param {number} per - Items per page (default 15)
 * @returns {Promise<Object>} PayHero response
 */
export async function getAccountTransactions(page = 1, per = 15) {
    try {
        ...
        console.log(`Fetching PayHero transactions: page=${page}, per=${per}`);

        const response = await fetch(
            `${PAYHERO_API_BASE_URL}/transactions?${queryParams}`,
            {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
        ...
        console.error("PayHero fetch transactions error:", error);
        return {
            success: false,
            error: error.message,
            transactions: [],
